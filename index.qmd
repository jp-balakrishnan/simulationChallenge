...
ax.set_title('Investment Simulation: Account Balance Over Coin Flips', fontsize=16, fontweight='bold')
...
print(f"  Number of coin flips: {N}")
...
print(f"  P(final balance ≥ ${2*starting_balance:,}):   {prob_double:.1%} ({np.sum(final_balances >= 2*starting_balance)}/{n_simulations} simulations)")
print(f"  P(final balance ≥ ${starting_balance:,}):    {prob_break_even:.1%} (break-even or better)")
...
print(f"   • The distribution is highly skewed with a long right tail")
print(f"   • Mean balance (${mean_balance:,.2f}) is {'higher' if mean_balance > median_balance else 'lower'} than median (${median_balance:,.2f})")
print(f"   • This indicates extreme outcomes: some simulations result in very high balances,")
print(f"     while others result in significant losses")
print(f"   • The wide range (${min_balance:,.2f} to ${max_balance:,.2f}) shows high variability")

print(f"\n2. PROBABILITY OF HAPPY OUTCOMES:")
print(f"   • After investing ${starting_balance:,}, the probability of ending with MORE than your initial")
print(f"     investment is {prob_above_buyin:.1%}")
print(f"   • This means in {np.sum(final_balances > starting_balance)} out of {n_simulations} simulations,")
print(f"     you would be profitable")
print(f"   • The probability of at least DOUBLING your money is {prob_double:.1%}")

print(f"\n3. RISK ASSESSMENT:")
if prob_above_buyin < 0.5:
    print(f"   • Less than half of simulations result in profit - this is a HIGH RISK investment")
elif prob_above_buyin < 0.6:
    print(f"   • Slightly more than half result in profit - this is a MODERATE RISK investment")
else:
    print(f"   • More than half of simulations result in profit - but still risky due to volatility")

print(f"   • The high standard deviation (${std_balance:,.2f}) indicates significant uncertainty")
print(f"   • You could lose as much as ${starting_balance - min_balance:,.2f} or gain as much as ${max_balance - starting_balance:,.2f}")

print(f"\n4. CONCLUSION:")
print(f"   • If you define 'happy outcomes' as finishing with more than your ${starting_balance:,} investment,")
print(f"     the probability is {prob_above_buyin:.1%}")
print(f"   • However, the extreme variability means outcomes are highly unpredictable")
print("="*80)
```
...
Instead of having the full balance at risk with each coin flip, assume only 50% of your balance is gambled each round. Compare this to the original game. Which is riskier? Which has better upside?
...
print("="*80)
if orig_std > mod_std:
    print(f"  • The ORIGINAL strategy is RISKIER (std dev: ${orig_std:,.2f} vs ${mod_std:,.2f})")
    print(f"    → Higher volatility means more uncertainty")
else:
    print(f"  • The MODIFIED strategy is RISKIER (std dev: ${mod_std:,.2f} vs ${orig_std:,.2f})")
    print(f"    → Higher volatility means more uncertainty")

if orig_max > mod_max:
    print(f"  • The ORIGINAL strategy has BETTER UPSIDE (max: ${orig_max:,.2f} vs ${mod_max:,.2f})")
    print(f"    → Can achieve much higher maximum returns")
else:
    print(f"  • The MODIFIED strategy has BETTER UPSIDE (max: ${mod_max:,.2f} vs ${orig_max:,.2f})")
    print(f"    → Can achieve much higher maximum returns")

print(f"\n  • The modified strategy (50% at risk) has:")
print(f"    - Lower risk per round (only 50% of balance exposed to coin flip)")
print(f"    - More conservative approach (50% always kept safe)")
print(f"    - Lower maximum potential but also lower minimum loss")
print(f"    - Reduced volatility compared to full-risk strategy")
print("="*80)
