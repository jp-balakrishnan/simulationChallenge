...
Expected Value Calculation: 0.5 * 1.5 * 30000 + 0.5 * 0.6 * 30000 = $31,500.00
...
# Show calculation and brief explanation using markdown format
EV = 0.5 * 1.5 * 30000 + 0.5 * 0.6 * 30000
print(f"Initial balance: ${30000:,.2f}")
print(f"Heads outcome: ${45000:,.2f}")
print(f"Tails outcome: ${18000:,.2f}")
print(f"Expected value after one flip: ${EV:,.2f}")
print(f"Gain in expected value: ${EV - 30000:,.2f} ({((EV - 30000) / 30000) * 100:.2f}%)")

```
#### Expected Value Analysis
This is an outcome for a single flip, not for the entire game. Here on each flip 50% gain and 40% loss, we can predict final outcome as profitable or not based on probability so you should not buy-in to the game based on expected value alone.
...
if outcome_assessment == "happy":
    assessment_text = "**✅ OUTCOME ASSESSMENT: I am HAPPY with this outcome. Its not profitable all time and its risky investment **"
...
* P(final balance ≥ ${2*starting_balance:,}): {prob_double:.1%} ({np.sum(final_balances >= 2*starting_balance)}/{n_simulations} simulations)
* P(final balance ≥ ${starting_balance:,}): {prob_break_even:.1%} (break-even or better)
